                                    ...
                                    <View style={{ paddingBottom: hp(2), height: hp(65), paddingVertical: hp(2.5) }}>
                                        <FlatList
                                            ...
                                            renderItem={({ item, index }) =>
                                            {
                                                ...
                                                let dd = textToSearch.length > 0 ? searchedData : props.pinnedMessage;
                                                let olddate = index <= 0 ? '' : dd[index - 1].created_at;
                                                
                                                // var isToday = moment(item.created_at).format('DD-MM-YYYY') === moment(currentdate).format('DD-MM-YYYY') ? true : false;
                                                let show = moment(newdate).format('DD-MM-YYYY') === moment(olddate).format('DD-MM-YYYY') ? false : true;

                                                return( 
                                                    moment(item.created_at).format('DD-MM-YYYY') === moment(currentdate).format('DD-MM-YYYY') ?
                                                    <DetailView show={show} item={item} index={index} />
   
                                            // PopularView(true, item, index, props.channel, props.status, textToSearch.length > 0 ? searchedData : props.pinnedMessage)
                                       :
                                      null
                                          )
                                         } 
